...
def compare2multiple(qimg_name, embed_dict):
	print("qimg_name", qimg_name)
	threshold = 1.10    # set yourself to meet your requirement
	compare_result_dict = {}
	compare_result_dict[qimg_name] = []
	for kf, vf in embed_dict.items():
		qimg_embed = embed_dict[qimg_name]
		dist = np.sqrt(np.sum(np.square(np.subtract(qimg_embed, vf))))	
		if dist <= threshold:
			compare_result_dict[qimg_name].append(kf)
	return compare_result_dict


print("compare result: ", compare2multiple(args.img1, file_embedding_dict))
